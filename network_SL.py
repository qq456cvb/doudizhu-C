            ...
            with tf.device('/gpu:0'):
                ...
                with slim.arg_scope([slim.fully_connected], weights_initializer=tf.contrib.layers.xavier_initializer()):
                    ...
                    with tf.variable_scope("last_cards"):
                        with tf.variable_scope("inputs"):
                            self.input_single_last = tf.placeholder(tf.float32, [None, 15], name="input_single_last")
                            self.input_pair_last = tf.placeholder(tf.float32, [None, 13], name="input_pair_last")
                            self.input_triple_last = tf.placeholder(tf.float32, [None, 13], name="input_triple_last")
                            self.input_quadric_last = tf.placeholder(tf.float32, [None, 13], name="input_quadric_last")

                        with tf.variable_scope("reshaping_for_conv"):
                            self.input_single_conv = tf.reshape(self.input_single_last, [-1, 1, 15, 1])
                            self.input_pair_conv = tf.reshape(self.input_pair_last, [-1, 1, 13, 1])
                            self.input_triple_conv = tf.reshape(self.input_triple_last, [-1, 1, 13, 1])
                            self.input_quadric_conv = tf.reshape(self.input_quadric_last, [-1, 1, 13, 1])
                    ...
                    with tf.variable_scope("passive_decision_making"):
                        ...
                        self.fc_decision_passive = tf.multiply(slim.fully_connected(self.fc_decision_passive, 1024, tf.nn.relu), self.attention_last_decision)
                    ...
                    with tf.variable_scope("passive_response"):
                        ...
                        self.fc_response_passive = tf.multiply(slim.fully_connected(self.fc_response_passive, 1024, tf.nn.relu), self.attention_last_response)
                        with slim.arg_scope([slim.fully_connected], activation_fn=tf.nn.relu):
                            self.fc_response_passive = slim.stack(self.fc_response_passive, slim.fully_connected, [256, 64])
                        self.fc_response_passive_output = slim.fully_connected(self.fc_response_passive, 15, tf.nn.softmax)
                ...
                with tf.variable_scope("active_loss"):
                    self.active_loss = tf.reduce_sum(self.active_decision_loss + self.active_response_loss + self.seq_length_loss * 5)

                with tf.variable_scope("total_loss"):
                    self.loss = tf.to_float(self.is_active) * self.active_loss + (1 - tf.to_float(self.is_active)) * self.passive_loss * 5

...
def identity_block(input, first_channel, last_channel, kernel_size):
    ...
    net = tf.nn.relu(net)
    residual = slim.conv2d(activation_fn=None, inputs=net, num_outputs=last_channel, biases_initializer=None,
                                            kernel_size=[1, kernel_size], stride=[1, 1], padding='SAME')

    residual = tf.contrib.layers.layer_norm(residual, scale=True)
    residual = tf.nn.relu(residual)
    residual = slim.conv2d(activation_fn=None, inputs=residual, num_outputs=last_channel, biases_initializer=None,
                                            kernel_size=[1, kernel_size], stride=[1, 1], padding='SAME')
...
def upsample_block(input, first_channel, last_channel, kernel_size):
    ...
    shortcut = slim.conv2d(activation_fn=None, inputs=net, num_outputs=last_channel, biases_initializer=None,

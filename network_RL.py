        ...
        with tf.variable_scope(scope):
            ...
            for i in range(gpus):
                ...
                with tf.variable_scope("value_loss"):
                    ...
                    self.value_loss = tf.reduce_sum(tf.square(self.value_input[i] - self.fc_value_output))
                ...
                self.var_norms.append(tf.global_norm(self.local_vars))
                self.gradients = tf.gradients(self.loss, self.local_vars)
                self.cliped_grads, grad_norms = tf.clip_by_global_norm(self.gradients, 400.0)
                ...
                with tf.control_dependencies(self.extra_update_ops):
                    with tf.variable_scope("optimize"):
                        self.optimize.append(trainer.apply_gradients(zip(self.cliped_grads, self.local_vars)))

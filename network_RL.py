            ...
            for i in range(gpus):
                with tf.device('/gpu:%d' % i):
                    ...
                    with tf.variable_scope("passive_response"):
                        ...
                        self.fc_response_passive_output.append(slim.fully_connected(self.fc_response_passive, 14, tf.nn.softmax))
                    ...
                    with tf.variable_scope("passive_mode_loss"):
                        self.passive_decision_input.append(tf.placeholder(tf.float32, [None, 4], name='passive_decision_in'))
                        self.passive_decision_loss = -tf.reduce_sum(self.passive_decision_input[i] * tf.log(
                            tf.clip_by_value(self.fc_decision_passive_output[i], 1e-10, 1 - (1e-10))), 1)

                        self.passive_response_input.append(tf.placeholder(tf.float32, [None, 14], name='passive_response_in'))
                        self.passive_response_loss = -tf.reduce_sum(self.passive_response_input[i] * tf.log(
                            tf.clip_by_value(self.fc_response_passive_output[i], 1e-10, 1 - (1e-10))), 1)

                        self.passive_bomb_input.append(tf.placeholder(tf.float32, [None, 13], name='passive_bomb_in'))
                        self.passive_bomb_loss = -tf.reduce_sum(self.passive_bomb_input[i] * tf.log(
                            tf.clip_by_value(self.fc_bomb_passive_output[i], 1e-10, 1 - (1e-10))), 1)

                    # active mode
                    with tf.variable_scope("active_mode_loss"):
                        self.active_decision_input.append(tf.placeholder(tf.float32, [None, 13], name='active_decision_in'))
                        self.active_decision_loss = -tf.reduce_sum(self.active_decision_input[i] * tf.log(
                            tf.clip_by_value(self.fc_decision_active_output[i], 1e-10, 1 - (1e-10))), 1)

                        self.active_response_input.append(tf.placeholder(tf.float32, [None, 15], name='active_response_in'))
                        self.active_response_loss = -tf.reduce_sum(self.active_response_input[i] * tf.log(
                            tf.clip_by_value(self.fc_response_active_output[i], 1e-10, 1 - (1e-10))), 1)

                        self.seq_length_input.append(tf.placeholder(tf.float32, [None, 12], name='sequence_length_in'))
                        self.seq_length_loss = -tf.reduce_sum(self.seq_length_input[i] * tf.log(
                                tf.clip_by_value(self.fc_sequence_length_output[i], 1e-10, 1 - (1e-10))), 1)
                    ...
                    cliped_grads, grad_norms = tf.clip_by_global_norm(gradients, 400.0)

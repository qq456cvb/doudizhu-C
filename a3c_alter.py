...
class CardNetwork:
    def __init__(self, s_dim, trainer, scope, a_dim=9085):
        with tf.variable_scope(scope):
            ...
            self.passive_decision_loss = -tf.reduce_sum(self.passive_decision_target * tf.log(self.fc_decision_passive_output), 1)
            ...
            self.passive_response_loss = -tf.reduce_sum(self.passive_response_target * tf.log(self.fc_response_passive_output), 1)
            ...
            self.passive_bomb_loss = -tf.reduce_sum(self.passive_bomb_target * tf.log(self.fc_bomb_passive_output), 1)
            ...
            self.active_decision_loss = -tf.reduce_sum(self.active_decision_target * tf.log(self.fc_decision_active_output), 1)
            ...
            self.active_response_loss = -tf.reduce_sum(self.active_response_target * tf.log(self.fc_response_active_output), 1)
...
if __name__ == '__main__':
    ...
    SLNetwork = CardNetwork(54 * 6, tf.train.AdamOptimizer(), "SLNetwork")
    e = env.Env()
    TRAIN = args.train
    sess = tf.Session()
    ...
    if TRAIN:
        ...
        for i in range(epoches_train):
            ...
            while r == 0:
                ...
                _, decision_passive_output, response_passive_output, bomb_passive_output, \
                    decision_active_output, response_active_output, minor_cards_output \
                     = sess.run([SLNetwork.optimize, SLNetwork.fc_decision_passive_output, 
                                ...
                                SLNetwork.fc_cards_value_output],
                        feed_dict = {
                ...
                # update accuracies
                minor_cards_acc_temp = 1 if np.array_equal(minor_cards_output.argsort()[:int(minor_cards_length[0])], \
                    minor_cards_target[0].argsort()[:int(minor_cards_length[0])]) else 0
                minor_cards_cnt += 1

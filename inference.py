...
def inference_minor_util(s, handcards, sess, network, num, is_pair):
    outputs = []
    ...
    for i in range(num):
        ...
        if is_pair:
            response_active_output[input_pair == 0] = 0
        else:
            response_active_output[input_single == 0] = 0
...
def inference_minor_cards(category, s, handcards, sess, network, seq_length):
    if category == Category.THREE_ONE.value:
        return inference_minor_util(s, handcards, sess, network, 1, False)
    if category == Category.THREE_TWO.value:
        return inference_minor_util(s, handcards, sess, network, 1, True)
    if category == Category.THREE_ONE_LINE.value:
        return inference_minor_util(s, handcards, sess, network, seq_length, False)
    if category == Category.THREE_TWO_LINE.value:
        return inference_minor_util(s, handcards, sess, network, seq_length, True)
    if category == Category.FOUR_TWO.value:
        return inference_minor_util(s, handcards, sess, network, 2, False)
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        saver.restore(sess, './Model/SL-0105/model-9950.ckpt')
        for i in range(100):
            e.reset()
            e.prepare()
            ...
            while not done:
                ...
                if idx == 2:
                    ...
                    input_single, input_pair, input_triple, input_quadric = get_masks(curr_cards_char, to_char(last_cards_value))
                    ...
                    if is_active:
                        ...
                        decision_active_output[decision_mask == 0] = 0
                        ...
                        response_active_output = response_active_output[0]
                        response_active_output[response_mask[decision_active] == 0] = 0
                        response_active = np.argmax(response_active_output)
                        ...
                        if active_category_idx == Category.SINGLE_LINE.value or \
                            ...
                            seq_length_output[length_mask[decision_active][response_active] == 0] = 0
                        ...
                        if active_category_idx == Category.THREE_ONE.value or \
                            ...
                            intention = np.concatenate([intention, to_value(inference_minor_cards(active_category_idx, s, curr_cards_char.copy(), sess, network, seq_length))])
                    else:
                        is_bomb = False
                        ...
                        decision_passive_output[decision_mask == 0] = 0
                        ...
                        if decision_passive == 0:
                            ...
                            bomb_passive_output[bomb_mask == 0] = 0
                            ...
                            response_passive_output[response_mask == 0] = 0
                            ...
                            if active_category_idx == Category.THREE_ONE.value or \
                                ...
                                intention = np.concatenate([intention, to_value(inference_minor_cards(last_category_idx, s, curr_cards_char.copy(), 
                                    sess, network, get_seq_length(last_category_idx, last_cards_value)))])
                    
                print("intention is ", end='')
